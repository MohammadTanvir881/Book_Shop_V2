...
const statusColorMap: Record<ApiOrderStatus, 'info' | 'primary' | 'success' | 'error'> = {
  Processing: 'info',
  Shipped: 'primary',
  Delivered: 'success',
  Cancelled: 'error'
...
const OrderEdit = () => {
 ...
 const { data: orderData, isLoading, refetch } = useGetOrderByIdQuery(id!);
  ...
  useEffect(() => {
    if (order) {
      ...
      const validStatuses: ApiOrderStatus[] = ['Processing', 'Shipped', 'Delivered', 'Cancelled'];
      const formattedStatus = validStatuses.includes(order.status as ApiOrderStatus) 
 ...
 const onSubmit = async (formData: OrderFormData) => {
  ...
  if (isLoading) {
    return (
      <Box p={4}>
        <Typography variant="h4" gutterBottom>Loading Order...</Typography>
        <Card sx={{ p: 3 }}>
        ...
        <Box mb={4}>
          ...
          <Box display="flex" flexDirection="column" gap={1}>
            ...
            <Box display="flex" alignItems="center" gap={1}>
              ...
              <Chip 
                label={order.status} 
                color={statusColorMap[order.status as ApiOrderStatus] || 'default'}
                size="small"
              />
            </Box>
          </Box>
